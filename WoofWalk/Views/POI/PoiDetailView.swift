...
struct PoiDetailView: View {
    ...
    var body: some View {
        ...
        .alert("Report POI", isPresented: $viewModel.showingReportSheet) {
            Button("Not Here") {
                Task { await viewModel.reportPoiMissing() }
            }
            Button("Inappropriate") {
                Task { await viewModel.reportPoi(reason: "Inappropriate content") }
            }
    ...
    private var votingButtons: some View {
        HStack(spacing: 24) {
            Button {
                Task { await viewModel.votePoi(upvote: true) }
            } label: {
                VStack {
            ...
            Button {
                Task { await viewModel.votePoi(upvote: false) }
            } label: {
                VStack {
