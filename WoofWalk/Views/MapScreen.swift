...
struct MapScreen: View {
    ...
    /// The walk-tracking ObservableObject the rest of MapScreen reads
    /// from. Bound to the real `WalkTrackingService.shared` (full Kalman
    /// + anchor-gate + pedometer-resume pipeline). The previous in-file
    /// toy `WalkTrackingViewModel` has been deleted; the legacy property
    /// names it exposed (`isWalkActive`, `walkDistance`, `walkDuration`,
    /// `startWalk()`, `stopWalk()`, `updateLocation(_:)`) are now
    /// provided as a thin convenience surface in
    /// `WalkTrackingService+MapScreen.swift`.
