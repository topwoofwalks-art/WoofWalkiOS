...
class PoiRepository {
    ...
    func votePoi(poiId: String, upvote: Bool) async throws {
        ...
        let countField = upvote ? "voteUp" : "voteDown"
        let addArray = upvote ? "upVotedBy" : "downVotedBy"
        let removeArray = upvote ? "downVotedBy" : "upVotedBy"

        try await db.collection("pois").document(poiId).updateData([
            countField: FieldValue.increment(Int64(1)),
            addArray: FieldValue.arrayUnion([userId]),
            removeArray: FieldValue.arrayRemove([userId])
        ])
    ...
    func reportPoiMissing(poiId: String) async throws {
        ...
        try await db.collection("pois").document(poiId).updateData([
            ...
            "removalReports": FieldValue.increment(Int64(1))
        ])
