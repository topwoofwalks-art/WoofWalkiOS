...
class LocalUserRepository {
    ...
    func updateWalkStreak(userId: String, streak: WalkStreak) throws {
        if let user = try getUserById(userId) {
            let encoder = JSONEncoder()
            user.walkStreakJson = (try? encoder.encode(streak)).flatMap { String(data: $0, encoding: .utf8) }
            try modelContext.save()
        }
    }

    func updateLeagueInfo(userId: String, tier: String?, groupId: String?, weekId: String?) throws {
        if let user = try getUserById(userId) {
            user.leagueTier = tier
            user.leagueGroupId = groupId
            user.leagueWeekId = weekId
            try modelContext.save()
        }
    }
}
